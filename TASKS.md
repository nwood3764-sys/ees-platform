...
- [x] **Scheduling — `book-appointment` edge function + `book_appointment` RPC** (this commit, 8 migrations) — Customer-facing booking endpoint deployed at `https://flyjigrijjjtcsvpgzvk.supabase.co/functions/v1/book-appointment` (public, `verify_jwt=false`). Takes a slot chosen from a compute-availability response plus customer details, atomically creates the full 10-record cascade via the `book_appointment` PG function: Account → Contact → Property → Building → Unit → Opportunity → Project → Work Order → Service Appointment → SAA → Booking Token. Concurrency: `pg_advisory_xact_lock` on (resource_id, calendar_day) serializes simultaneous bookings against the same Tech; post-lock conflict check rejects duplicate slots with `status='slot_taken'`. Input validation: phone normalized to 10 digits (drops formatting and leading "1" country code), ZIP to 5 digits, state to 2-char code — all matching schema CHECK constraints. Dedup: Contact lookup by phone; if found, reuses the Account. Token: 32 hex chars from `gen_random_uuid()`, used in customer manage URL `<APP_BASE_URL>/book/manage/<token>`, expires 7 days after appointment start. Multifamily Energy Assessment skill requirement also updated to BPI Multifamily Building Analyst (previously BPI Building Analyst — single-family-focused auditor isn't qualified for 4-plex). Final qualification matrix: Single-Family/Townhome → Javier + Kenji (BPI BA); MF Energy + MF Diagnostic → Kenji alone (BPI MFBA). Smoke test verified: HTTP POST through edge function creates full cascade with `manage_url`; duplicate attempt returns `slot_taken`. Schema gotchas discovered: `contacts.contact_name` is NOT NULL alongside first/last (set = `first||' '||last`); `gen_random_bytes` requires pgcrypto (not enabled), used `gen_random_uuid` instead; `booking_tokens` is a lean table with no record_number/owner/created_by. Notification firing deferred — Twilio A2P 10DLC and Mail.Send permission still pending. Migrations: scheduling_book_appointment_rpc + 6 iterative fixes (lock pattern, created_at prefix, contact_name, phone/zip/state validation, UUID token, booking_tokens schema) + scheduling_mf_energy_assessment_requires_mfba. Final RPC consolidated in `supabase/migrations/20260515020000_scheduling_book_appointment_rpc.sql`.
