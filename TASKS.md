...
- [ ] **Customer-bookable work_types mapping** — six appointment types in the spec (Single-Family / Townhome / Multifamily / Multifamily Diagnostic Energy Assessment, HVAC Quote, Customer Consultation). Only assessments are customer-bookable per Nicholas; HVAC Quote and Consultation are dispatcher-only. Final mapping of which existing `work_types` rows to extend (vs net-new rows) still being worked through.
...
- [ ] **`send-email` edge function** — Microsoft Graph wrapper using application-permission Mail.Send scoped to `scheduling@ees-wi.org` shared mailbox. Writes to `notification_logs` with `nl_provider='microsoft_graph'` and Graph message ID. Blocked until Nicholas creates the shared mailbox + Azure AD app registration.
...
- [blocked] **`scheduling@ees-wi.org` shared mailbox + Azure AD app registration** — Mail.Send application permission via PowerShell `Add-MailboxPermission` + `Add-RecipientPermission`. Nicholas's parallel workstream.
