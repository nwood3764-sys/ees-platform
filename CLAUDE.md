...
- **Dashboard rehaul is the QUEUED NEXT WORKSTREAM (Nicholas, 2026-07-26: "enterprise class… 25 or more components… full rehaul"):** complete research-backed spec in `docs/leap-dashboard-rehaul.md` — vendor component synthesis (Power BI/Salesforce/Tableau/Grafana → 38-type master list), Salesforce per-widget/dashboard parity checklist, phased plan (A: port 9 renderers to themed ECharts w/ fill-height/labels/legends/formats/conditional colors; B: four new query-shape RPCs; C: expand to 25+ components; D: subscriptions/drill/view-as). Key recommendations pending Nicholas's yes/no: **Apache ECharts** engine (Apache-2.0, DB-storable JSON themes) over Recharts, **TanStack Table** over AG Grid Community (grouping/aggregation is AG Enterprise-paywalled). Advisor baseline note: security lints now total ~205 (drift from recent sessions' RPCs — same known categories, nothing new from this session's DDL).
