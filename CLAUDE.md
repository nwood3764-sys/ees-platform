...
**Shipped 2026-07-26 — Related-record (two-hop) related lists in page layouts (branch `claude/related-list-field-selection-jeb2f0`, PR #212; DB live on prod):**
Nicholas wanted a Units related list on the Property page — impossible before, because related lists required a direct FK and units link to buildings, not properties. Now any object can expose grandchild records:
- New RPC `describe_object_grandchild_fks(p_table)` (migration `20260726150000`) returns every two-hop FK path; the Add Related List modal's Target Table dropdown gained a "Via a related record" group (`units · via buildings` — targets with a direct FK excluded from the group), showing a read-only Join Path instead of the FK picker and emitting `config.via = {table, fk}` alongside `config.fk` (the target's FK to the via table).
- `fetchRelatedRecords` resolves via configs as a PostgREST inner-join embed (`_via:<fk>!inner(<via.fk>)` + `.eq('_via.<via.fk>', parentId)`) — one round trip, RLS-respecting, count-accurate; `_via` stripped from rows. Via lists hide the New button (the child's FK points at the intermediate record — create from the intermediate's page); rows navigate normally. `validate_page_layout_widget_config` needed no change (it checks `fk` exists on the target table, not where it points).
- Also fixed in the modal: soft-delete filter column is now detected from the target's real columns (`unit_is_deleted` etc. — the old blanket `'is_deleted'` default 400'd fetches on prefix-audited tables), and stale display/sort picks are dropped when the target table changes. Help article **HA-00143** (`related-lists-via-related-records`).
