...
Active workstream: **LEAP Pad field-service work orders** — technician-created work orders, evidence pipeline quality, verification routing (PRs #114–#133, all live on master/prod; Nicholas mid-testing). Previous workstream (Builder rearchitecture — WYSIWYG builders, `docs/leap-builder-rearchitecture.md`) is functionally complete.
...
- **Project Paperwork port — Phase 1 SHIPPED 2026-07-26** (`docs/leap-project-paperwork-port.md`): `generate_paperwork` record action on projects → `ProjectPaperworkModal` generates all five HOMES documents + workbook as downloads (`paperworkModel.js` pure math node-tested against the Hampton fixture, `paperworkService.js` record prefill incl. account billing address + property IQ number, workbook template at `public/paperwork/`). Help article **HA-00144** (`generating-homes-project-paperwork`). Phases 2 (persist to record: invoice-number fields, auto-attach documents) + 3 (LEAP-native quantities) remain — see doc §7/§9.
