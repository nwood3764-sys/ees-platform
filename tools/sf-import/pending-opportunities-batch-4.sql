-- Opportunities layouts batch 4/4 (6 layouts)
DO $$
DECLARE
...
-- Layout: WI-IRA-MF-HOMES-Audit  →  RT picklist_value = WI_IRA_MF_HOMES_Audit
  SELECT id INTO rt_id FROM public.picklist_values WHERE picklist_object = 'opportunities' AND picklist_field = 'record_type' AND picklist_value = 'WI_IRA_MF_HOMES_Audit' LIMIT 1;
  ...
  INSERT INTO public.page_layouts (
    ...
    '', 'WI-IRA-MF-HOMES-Audit', 'opportunities',
    'record_detail', true,
    'Imported from SF Layout: Opportunity-WI-IRA-MF-HOMES-Audit',
    rt_id, fb, fb, false
  ) RETURNING id INTO layout_id;
...
-- Layout: WI-IRA-SF-HOMES-AUDIT  →  RT picklist_value = WI_IRA_SF_HOMES_AUDIT
  SELECT id INTO rt_id FROM public.picklist_values WHERE picklist_object = 'opportunities' AND picklist_field = 'record_type' AND picklist_value = 'WI_IRA_SF_HOMES_AUDIT' LIMIT 1;
  ...
  INSERT INTO public.page_layouts (
    ...
    '', 'WI-IRA-SF-HOMES-AUDIT', 'opportunities',
    'record_detail', true,
    'Imported from SF Layout: Opportunity-WI-IRA-SF-HOMES-AUDIT',
    rt_id, fb, fb, false
  ) RETURNING id INTO layout_id;
  ...
  INSERT INTO public.page_layout_widgets (
    ...
    jsonb_build_object('fields', jsonb_build_array(
        ...
        jsonb_build_object('name', 'opportunity_stage_notes_del', 'label', 'Stage Notes Del'),
        jsonb_build_object('name', 'opportunity_owner', 'label', 'Owner'),
        jsonb_build_object('name', 'opportunity_jobber_url', 'label', 'Jobber Url'),
        jsonb_build_object('name', 'opportunity_snugg_pro_url', 'label', 'Snugg Pro Url')
      )), false
  );
  ...
  INSERT INTO public.page_layout_widgets (
    ...
    jsonb_build_object('fields', jsonb_build_array(
        jsonb_build_object('name', 'opportunity_homes_application_submitted', 'label', 'Homes Application Submitted'),
        jsonb_build_object('name', 'opportunity_homes_application_approved', 'label', 'Homes Application Approved'),
        jsonb_build_object('name', 'opportunity_link_to_program_incentive_payment_applic', 'label', 'Link To Program Incentive Payment Applic')
      )), false
  );
