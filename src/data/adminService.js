...
export async function fetchHomePages() {
  const { data, error } = await supabase
    .from('home_pages')
    .select('id, hp_name, hp_template, hp_role_id, hp_module_id, hp_is_active, hp_is_default, hp_updated_at')
...
export async function fetchHomePage(pageId) {
  const [{ data: page, error: e1 }, { data: comps, error: e2 }] = await Promise.all([
    supabase.from('home_pages').select('id, hp_name, hp_template, hp_role_id, hp_module_id, hp_is_active, hp_is_default').eq('id', pageId).single(),
...
export async function saveHomePage(page, components) {
  const { data, error } = await supabase.rpc('save_home_page', {
    p_page: {
      ...
      role_id: page.roleId || null, module_id: page.moduleId || null, is_active: !!page.isActive, is_default: !!page.isDefault,
    },
