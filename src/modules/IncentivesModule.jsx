          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:8, overflow:'hidden' }}>
            ...
            <div style={{ padding:'12px 14px', display:'flex', gap:10, alignItems:'center' }}>
              <R.ResponsiveContainer width={80} height={80}>
                <R.PieChart><R.Pie data={prByStatus} cx="50%" cy="50%" innerRadius={18} outerRadius={36} dataKey="value" strokeWidth={0}>{prByStatus.map((_,i)=><R.Cell key={i} fill={seriesColor(i)}/>)}</R.Pie></R.PieChart>
    ...
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div data-module-topbar="1" style={{ height: 54, background:C.card, borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 24px', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:6, fontSize:13 }}>
          <NavLink to={{ activeModule: 'incentives' }} onActivate={() => { closeRecord(); setSec(SECTIONS[0]?.id) }} style={{ color:C.emeraldMid, cursor:'pointer', fontWeight:500 }}>Incentives</NavLink><span style={{ color:C.textMuted }}>/</span>
