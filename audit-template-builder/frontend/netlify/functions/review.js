...
- Measure naming schema: CustomMeasureName is a clean identifier with per-block suffixes stripped (never "... for Block in block for ..."); identical per-block measures are merged into ONE measure; the roof measure's description enumerates every distinct baseline roof R ("attic improved from R-2 to R-49, and R-19 to R-49") and appends "insulate knee walls to R-15" when the baseline report names an exposed knee wall.
- All-electric (or gas-only) buildings: a fuel whose annual total is zero must be ABSENT from the file — no meter or annual-summary scenario, no ResourceUse rows, no package savings row for that fuel. A Natural Gas section with reference-building therms (or even zeros) on an all-electric building is a mismatch. Electric water heaters use ElectricResistance (never Combustion) with PrimaryFuel Electricity.
- Savings package: exactly ONE package (default name "IRA HOMES") whose AnnualSavingsByFuel values are the whole-building baseline − improved totals (electricity kWh and natural gas therms, matching the meter deltas) — never the reference file's scaled savings. The user may edit measures and package savings on the page; the digest reflects those edits, so audit for internal consistency (savings ≈ baseline meter total − improved meter total per fuel; flag large unexplained deviations as a question, not a mismatch).
