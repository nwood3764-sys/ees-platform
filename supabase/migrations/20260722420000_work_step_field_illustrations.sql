-- =====================================================================
-- Per-field example illustrations for screen-flow prompts (Nicholas,
-- 2026-07-22): every photo prompt can carry a line-drawing example the
-- auditor taps to see what the picture must include ("the furnace, the
-- ductwork, and surrounding areas"). wstf_illustration_url points at an
-- app asset (or any URL — admin-swappable); the technician detail RPC
-- surfaces it as `illustration`. Seeded for the six assessment photo
-- prompts (equipment + nameplate on Heating / Cooling / Water Heating);
-- SVGs live in public/illustrations/assessment/.
-- =====================================================================

ALTER TABLE public.work_step_template_fields
  ADD COLUMN IF NOT EXISTS wstf_illustration_url text;
COMMENT ON COLUMN public.work_step_template_fields.wstf_illustration_url IS
  'Optional example illustration (line drawing) shown with the prompt in the screen flow — app-asset path or full URL.';

UPDATE public.work_step_template_fields f
   SET wstf_illustration_url = v.url,
       wstf_updated_by = 'c5a01ec8-960f-42ab-8a9e-a49822de89af', wstf_updated_at = now()
  FROM (VALUES
    ('Heating System',       'equipment_photo', '/illustrations/assessment/heating-equipment-photo.svg'),
    ('Heating System',       'nameplate_photo', '/illustrations/assessment/heating-nameplate-photo.svg'),
    ('Cooling System',       'equipment_photo', '/illustrations/assessment/cooling-equipment-photo.svg'),
    ('Cooling System',       'nameplate_photo', '/illustrations/assessment/cooling-nameplate-photo.svg'),
    ('Water Heating System', 'equipment_photo', '/illustrations/assessment/water-heating-equipment-photo.svg'),
    ('Water Heating System', 'nameplate_photo', '/illustrations/assessment/water-heating-nameplate-photo.svg')
  ) AS v(step_name, field_name, url),
  public.work_step_templates t
 WHERE t.id = f.work_step_template_id AND t.wst_name = v.step_name AND t.wst_is_deleted IS NOT TRUE
   AND f.wstf_field_name = v.field_name AND f.wstf_is_deleted IS NOT TRUE;

-- work_order_detail_for_technician: fields gain `illustration`.
-- (Full body — identical to the prior version plus that one key.)
