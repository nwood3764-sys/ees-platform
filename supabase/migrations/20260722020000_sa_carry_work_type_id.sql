-- Self-scheduled service appointments were not carrying work_type_id: the
-- create_service_appointment RPC resolved the work type from the public slug
-- and stamped it on the work order, but the service_appointments INSERT
-- omitted the column. That left every self-booked SA with a NULL work type,
-- so confirmation emails, the LEAP Pad Today view, and reporting all fell back
-- to a generic "site visit" label instead of the real work type name.
--
-- This migration:
--   1. Recreates create_service_appointment with work_type_id set on the SA.
--   2. Backfills work_type_id on existing SAs from their linked work order.
...
BEGIN
  ...
  -- SA carries work_type_id so downstream (confirmation emails, LEAP Pad Today
  -- view, reporting) sees the real work type instead of a generic fallback.
...
-- Backfill: existing self-scheduled SAs whose work type is NULL inherit it
-- from their linked work order.
UPDATE service_appointments sa
   SET work_type_id = w.work_type_id,
       sa_updated_at = now()
  FROM work_orders w
 WHERE sa.work_order_id = w.id
   AND sa.work_type_id IS NULL
   AND w.work_type_id IS NOT NULL
   AND coalesce(sa.sa_is_deleted, false) = false;
