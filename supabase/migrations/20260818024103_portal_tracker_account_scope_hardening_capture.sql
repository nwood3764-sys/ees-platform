-- =============================================================================
-- Project Portal — capture the account-scope hardening that only ever existed
-- in production.
--
-- Prod's migration registry carries `portal_tracker_account_scope_hardening`
-- (applied 2026-07-01) but no file for it was ever committed, so replaying this
-- repo onto a fresh branch database produced the PRE-hardening tracker: one
-- without the `p.property_account_id = v_account_id` guard on granted
-- properties/buildings, and with the old `status = 'Active'` gate. That is a
-- silent security regression on every sandbox, and would become one in prod if a
-- branch were ever promoted.
--
-- This file is a verbatim capture of the live production definition of
-- get_portal_project_tracker() (byte-verified against pg_get_functiondef before
-- committing). Applying it to prod is a no-op; applying it to a branch brings
-- the branch up to prod.
-- =============================================================================
