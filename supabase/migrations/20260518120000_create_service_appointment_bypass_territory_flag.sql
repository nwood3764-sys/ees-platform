-- ─── create_service_appointment v2 ────────────────────────────────────
-- Adds the bypass_territory_check flag (DFR → SA conversion v1.1).
--
-- The flag is read from payload but ONLY honored when
-- current_app_user_id() IS NOT NULL — i.e., when the caller has an
-- authenticated app-user session. The customer-facing path calls this
-- RPC via the public edge function using the service-role key, where
-- auth.uid() is NULL, so the bypass cannot be activated from anon
-- callers. No privilege escalation surface.
--
-- When v_bypass_territory is true and the customer's ZIP doesn't match
-- any active service_territory_zips row, the cascade proceeds with
-- service_territory_id = NULL (the column is already nullable). All
-- other validation (phone format, ZIP shape, state shape, work_type
-- slug existence, slot conflict via advisory lock) remains exactly
-- as before.
--
-- This is a surgical change: only two diffs from v1.
--   1. New v_bypass_territory boolean declared + computed early
--   2. The "Address is outside our service territory" guard now
--      guards on `v_territory_id IS NULL AND NOT v_bypass_territory`
-- Plus one return-shape addition: territory_bypassed flag in the OK
-- response so the frontend can adjust the success toast accordingly.
--
-- All other logic (record cascade, advisory lock, conflict detection,
-- token generation, owner stamps) is byte-identical to v1.
--
-- VERIFIED IN PROD (live smoke):
--   • Authenticated dispatcher + bypass=true + ZIP 99999 (no territory):
--     RPC returns status=ok, service_territory_id NULL on the SA,
--     territory_bypassed=true. SA was BypassSmoke Test cleanup row.
--   • Anon caller + bypass=true + ZIP 99999:
--     RPC returns status=error, message='Address is outside our
--     service territory.' — bypass silently ignored as designed.
