-- =============================================================================
-- Project Portal — building-level opportunity tracker
--
-- Corrects the model: the portal is driven entirely by OPPORTUNITY statuses,
-- and opportunities are BUILDING-level. A property has no status of its own —
-- it is just a container that holds buildings, and each building carries its
-- own opportunities (one per record type, e.g. an IRA HOMES and an IRA HEAR
-- opportunity), each with its data-driven stage list + current stage_order.
-- No unit tier, no property-level status.
--
-- get_portal_project_tracker() returns:
--   Property (container) → Building → Opportunity[{stage_label, stage_order,
--   stages:[{label, sort_order}]}]
-- =============================================================================
