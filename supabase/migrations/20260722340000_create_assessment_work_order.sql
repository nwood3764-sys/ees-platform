-- =====================================================================
-- Self-service "New Assessment" (Nicholas, 2026-07-22). An auditor in
-- LEAP Pad creates a Single-Family Energy Assessment work order and, when
-- needed, the records behind it:
--   * Account — the BUILDING OWNER (owner-occupied or rental): selected
--     from existing accounts or created by name (record type
--     property_owner).
--   * Property — selected via search, or created from street/city/state/
--     zip under the chosen account.
--   * Building — selected, or created with a record type the auditor
--     picks: Single Family Detached (no units — work orders live at
--     building level) or Single Family Attached (each attached home is a
--     unit). Building naming = street number (LEAP convention).
--   * Unit — required ONLY for attached buildings: pick an existing unit
--     or type a new one. Detached assessments carry no unit.
--   * Opportunity/Project — find-or-create per property, purpose-built:
--     NEW opportunity record type single_family_energy_assessment with
--     its own never-shared stage picklist (scoped via pvrta), and the
--     existing project record type single_family_energy_assessment.
--   * Work order + service appointment (now → +3h) assigned to the
--     creating auditor, so it lands on their Today view immediately.
-- The work plan instantiates via the existing WO-insert trigger.
-- =====================================================================

-- ── Purpose-built opportunity record type + stages (never shared) ─────────────
DO $$
DECLARE
...
BEGIN
  SELECT id INTO v_rt FROM public.picklist_values
   WHERE picklist_object='opportunities' AND picklist_field='record_type'
     AND picklist_value='single_family_energy_assessment';
  IF v_rt IS NULL THEN
    ...
    VALUES ('opportunities','record_type','single_family_energy_assessment','Single-Family Energy Assessment', true,
  ...
  SELECT id INTO v_open FROM public.picklist_values
   WHERE picklist_object='opportunities' AND picklist_field='opportunity_stage'
     AND picklist_value='Opportunity — Single-Family Assessment Open';
  IF v_open IS NULL THEN
    INSERT INTO public.picklist_values (picklist_object, picklist_field, picklist_value, picklist_label, picklist_is_active, picklist_sort_order, picklist_created_by)
    VALUES ('opportunities','opportunity_stage','Opportunity — Single-Family Assessment Open','Opportunity — Single-Family Assessment Open', true, 10, v_nick)
    RETURNING id INTO v_open;
  END IF;

  SELECT id INTO v_done FROM public.picklist_values
   WHERE picklist_object='opportunities' AND picklist_field='opportunity_stage'
     AND picklist_value='Opportunity — Single-Family Assessment Completed';
  IF v_done IS NULL THEN
    INSERT INTO public.picklist_values (picklist_object, picklist_field, picklist_value, picklist_label, picklist_is_active, picklist_sort_order, picklist_created_by)
    VALUES ('opportunities','opportunity_stage','Opportunity — Single-Family Assessment Completed','Opportunity — Single-Family Assessment Completed', true, 20, v_nick)
...
CREATE OR REPLACE FUNCTION public.create_assessment_work_order(
  ...
  p_new_building_type text DEFAULT NULL,   -- 'single_family_detached' | 'single_family_attached'
...
BEGIN
  ...
  -- The assessment work type (purpose-built; not gated on technician-creatable).
  ...
  ELSE
    ...
    END IF;
    -- Dedup: same street+zip under this account.
  ...
  -- ── Building: select, or create with the chosen single-family type ─────────
  ...
  -- Building type drives the unit rule.
  ...
  -- ── Unit: required for attached, absent for detached ───────────────────────
  IF v_bld_rt = 'single_family_detached' THEN
    v_unit := NULL; v_unit_label := NULL;  -- the building IS the home
  ...
  -- ── Work order + appointment, assigned to the auditor ──────────────────────
  ...
  -- Field Data Verification Review when new records were created.
