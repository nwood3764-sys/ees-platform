-- =============================================================================
-- Project Portal — work steps under each work order
--
-- Final execution layer: Work Order → Work Step[]. Each work order now nests its
-- work steps (work_steps.work_order_id) in execution order, each with its status
-- (work_step_status → label) and reference photo URL (work_step_reference_photo_url,
-- surfaced now so the portal can expose work-step photos later). Full chain:
--   Property → Building → Opportunity → Project → Work Order → Work Step
-- =============================================================================
  ...
  ws_by_wo AS (
    SELECT work_order_id,
      jsonb_agg(jsonb_build_object(
        'id', id, 'name', work_step_name, 'status', status_label, 'order', ord, 'photo_url', photo_url
      ) ORDER BY ord) AS steps
    FROM scoped_work_steps GROUP BY work_order_id
  ),
  wo_by_project AS (
