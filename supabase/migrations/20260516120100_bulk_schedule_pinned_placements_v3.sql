-- bulk_schedule_work_orders v3: pinned placements.
--
-- New optional param p_pinned_placements jsonb = '[{"work_order_id": uuid,
-- "start_ts": iso-string}, ...]'. Each pinned WO is placed at its exact
-- start time before the greedy loop runs; non-pinned WOs flow around them
-- as if the pin were a calendar absence on the Team Lead.
--
-- Pinning rules:
--   * A pinned WO is placed at p_start_ts exactly, end = start + duration.
--   * The (start, end) interval is carved out of the day's working
--     periods just like an existing SA or resource_absence -- so all later
--     non-pinned placements respect it.
--   * If the pin falls outside the day's working window (before 7am,
--     after 3:30pm, during lunch, on a weekend, outside [p_start_date,
--     p_end_date]), the WO is returned unplaced with placement_error
--     'pin_outside_working_hours'.
--   * If two pins overlap each other on the same lead, the LATER pin is
--     returned unplaced with placement_error 'pin_overlaps_existing'.
--     The dispatcher needs to fix it.
--   * Pinned WO retains placed=true in the result, scheduled_start_iso
--     reflects the pin.
--
-- Per-WO buffer + same-unit-zero behavior still applies to non-pinned
-- placements walking the input array. The walker SKIPS pinned WOs since
-- they're already placed; same-unit peek-ahead also skips pinned WOs
-- when deciding zero-vs-default buffer.
...
BEGIN
  ...
  IF jsonb_array_length(p_pinned_placements) > 0 THEN
    ...
    LOOP
      ...
      FOR i IN 1..v_wo_count LOOP
        IF p_work_order_ids[i] = v_pin.wo_id THEN
          v_pin_idx := i; EXIT;
        END IF;
      END LOOP;
      ...
      FOR v_other_idx IN 1..v_wo_count LOOP
        IF v_wo_is_pinned[v_other_idx]
           AND v_placed_starts[v_other_idx] IS NOT NULL
           AND v_placed_ends[v_other_idx]   > v_pin_start
           AND v_placed_starts[v_other_idx] < v_pin_end
        THEN
          v_pin_overlaps := true; EXIT;
        END IF;
      END LOOP;
      ...
      FOR i IN 1..v_wo_count LOOP
        ...
        IF v_fit_idx IS NOT NULL THEN
          ...
          FOR m IN i+1..v_wo_count LOOP
            IF v_placed_starts[m] IS NULL
               AND v_placement_errors[m] IS NULL
               AND NOT v_wo_is_pinned[m]
            THEN
  ...
  RETURN QUERY
    SELECT
      ...
      (v_placed_starts[gs.idx] IS NOT NULL AND v_placement_errors[gs.idx] IS NULL) AS placed,
...
'Project Scheduler engine v3. Greedy first-fit allocator with per-work-type post-buffer, same-unit zero-buffer override, AND pinned placements (p_pinned_placements jsonb array of {work_order_id, start_ts}). Pinned WOs lock to their exact start; non-pinned WOs flow around them like absences. p_commit=false returns the preview plan; p_commit=true writes SAs/SAAs/WO status under an advisory lock on the team lead.';
