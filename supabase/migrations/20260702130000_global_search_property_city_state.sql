-- Properties search results: show city and state in the secondary label.
-- Same requirement as the buildings change (20260702120000): the search
-- subtitle must tell the user what city/state a record is in. Property rows
-- showed only property_aka_name; now "aka name — city, state" (aka omitted
-- when blank, location omitted when blank). Applied to both global_search
-- and fuzzy_match_records so the exact-match and "did you mean" surfaces
-- agree. Everything else in both functions is unchanged.
...
AS $function$
  ...
  q_properties AS (
    SELECT 'property'::text, 'Properties'::text, 'properties'::text, p2.id,
           p2.property_name,
           NULLIF(CONCAT_WS(' — ',
             NULLIF(TRIM(p2.property_aka_name), ''),
             NULLIF(CONCAT_WS(', ',
               NULLIF(TRIM(p2.property_city), ''),
               NULLIF(TRIM(p2.property_state), '')
             ), '')
           ), ''),
           p2.property_record_number,
           CASE
