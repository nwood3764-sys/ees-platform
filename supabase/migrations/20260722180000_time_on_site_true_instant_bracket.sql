-- Time-on-site bracket: use photos.created_at (a true UTC instant) instead of
-- photos.taken_at (the camera's LOCAL wall-clock time, which exifr stores
-- labelled UTC because it does not apply EXIF OffsetTimeOriginal).
--
-- Why this matters: the LEAP Pad "Time on site" card computes the running
-- total as (now - first_photo_at) on the client. `now` is a true instant, but
-- first_photo_at was min(taken_at) — a wall clock mislabelled UTC, so for a
-- 9:44 AM Central capture it read 09:44:51Z instead of the true 14:44:51Z.
-- Subtracting that from a true `now` overstated the running total by exactly
-- the UTC offset (e.g. 8h 51m shown for a real 3h 51m on site). The completed
-- total (last - first) happened to be correct because both endpoints carried
-- the same offset and it cancelled, but the running total did not.
--
-- photos.created_at is the server-set insert time (the upload moment, within a
-- few seconds of capture) and is a genuine timestamptz instant, so it is the
-- correct basis for elapsed-time math and is timezone-safe in every state EES
-- operates in. Per-photo display still uses taken_at (the device wall clock),
-- which is what a technician expects to see.
--
-- Only the v_first/v_last bracket changes; the rest of the function is the
-- live production definition verbatim.
...
BEGIN
  ...
  -- Evidence-bracketed duration: first and last photo across all steps on this
  -- WO, by created_at (true server instant) so elapsed math is timezone-safe.
