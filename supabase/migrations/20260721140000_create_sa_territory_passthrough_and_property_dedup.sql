-- =============================================================================
-- create_service_appointment: territory passthrough + property de-duplication
--
-- Two additive changes to the customer-facing booking cascade:
--
--   1. Honor a client-supplied territory_id. compute-availability already
--      resolves the territory (by polygon or ZIP) and returns territory.id;
--      the booking now trusts that when present, so an address that
--      availability approved never gets rejected at booking time for lack of a
--      seeded ZIP row. Falls back to the existing ZIP lookup when absent.
--
--   2. Reuse an existing property instead of creating a duplicate. When the
--      homeowner is already known (contact matched by phone → existing
--      account) and a property with the same account + street + ZIP already
--      exists, the booking attaches to it (and its first building/unit) rather
--      than creating a second property record for the same address. A brand
--      new customer is unaffected — their fresh account has no properties, so
--      a new one is created exactly as before. Opportunity / project / work
--      order / service appointment are still created fresh per booking.
--
-- Everything else (advisory lock, conflict check, contact de-dup, token,
-- bypass path, exception handling) is preserved verbatim.
-- =============================================================================
...
BEGIN
  ...
  -- NEW: trust the territory compute-availability already resolved, when given.
  ...
  -- Fall back to the ZIP → territory lookup when no valid territory_id was passed.
  ...
  -- NEW: reuse an existing property for the same account + address rather than
  -- creating a duplicate. Only matches when the homeowner is already known;
  -- a fresh account has no properties, so a new one is created as before.
  ...
  -- Reuse the property's first building/unit when present; otherwise create.
