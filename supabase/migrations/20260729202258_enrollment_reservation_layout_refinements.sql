-- IRA HOMES Project Reservation enrollment layout — refined to mirror the
-- "IRA HOMES Multifamily Project Submittal" JotForm section-by-section.
--
-- Sections (two-column, matching the form order): Enrollment (internal) ->
-- Application -> Primary Contractor Information -> Support Contractor
-- Information (conditional field_group, shown only when a support contractor
-- is used) -> Installation Building Information -> Utility Information ->
-- Building Improvements (What Work + Improvement Costs) -> Payment Information
-- -> Supporting Documentation (typed upload tiles) -> Submission.
--
-- Sourcing: contractor name/email/phone/address are related fields off the
-- registered-contractor account; the contractor CONTACT picker is scoped to
-- that account (contacts_for_accounts). Installation address/units/floors pull
-- from the property and building records. Total Floors -> building stories.
DO $$
DECLARE
...
BEGIN
  ...
  END IF;
  UPDATE page_layout_widgets SET is_deleted=true, updated_at=now() WHERE page_layout_id=v_layout AND is_deleted IS NOT TRUE;
  UPDATE page_layout_sections SET is_deleted=true, deletion_reason='Layout refinements', updated_at=now() WHERE page_layout_id=v_layout AND is_deleted IS NOT TRUE;
  ...
  INSERT INTO page_layout_sections (id, page_layout_id, section_order, section_label, section_columns, section_tab, section_placement, is_deleted) VALUES
    ...
    (s_support, v_layout, 4, 'Support Contractor Information',    2, 'Details', 'main', false),
    (s_install, v_layout, 5, 'Installation Building Information', 2, 'Details', 'main', false),
    (s_utility, v_layout, 6, 'Utility Information',               2, 'Details', 'main', false),
    (s_improve, v_layout, 7, 'Building Improvements',             2, 'Details', 'main', false),
    (s_payment, v_layout, 8, 'Payment Information',               2, 'Details', 'main', false),
    (s_docs,    v_layout, 9, 'Supporting Documentation',          2, 'Details', 'main', false),
    (s_submit,  v_layout,10, 'Submission',                        1, 'Details', 'main', false);
  ...
  ('', v_layout, s_contr, 'field_group', 'Primary Contractor Information', 1, 1,
   '{"fields":[
      ...
      {"name":"enrollment_contractor_contact_id","type":"lookup","label":"Primary Contractor Contact Name","column":2,"lookup_table":"contacts","lookup_field":"contact_name","lookup_dependency":{"kind":"contacts_for_accounts","depends_on":["enrollment_contractor_account_id"]}},
      {"name":"enrollment_contractor_contact_id.contact_first_name","type":"related_field","label":"First Name","column":2,"related":{"table":"contacts","column":"contact_first_name","fk_column":"enrollment_contractor_contact_id","column_type":"text"}},
      {"name":"enrollment_contractor_contact_id.contact_last_name","type":"related_field","label":"Last Name","column":2,"related":{"table":"contacts","column":"contact_last_name","fk_column":"enrollment_contractor_contact_id","column_type":"text"}},
  ...
  ('', v_layout, s_support, 'field_group', 'Support Contractor Information', 1, 1,
   '{"visible_when":{"field":"enrollment_has_support_contractor","equals":true},"fields":[
      {"name":"enrollment_support_contractor_account_id","type":"lookup","label":"Support Contractor Business Name","column":1,"lookup_table":"accounts","lookup_field":"account_name"},
      {"name":"enrollment_support_contractor_contact_id","type":"lookup","label":"Support Contractor Contact Name","column":2,"lookup_table":"contacts","lookup_field":"contact_name","lookup_dependency":{"kind":"contacts_for_accounts","depends_on":["enrollment_support_contractor_account_id"]}},
      {"name":"enrollment_support_contractor_account_id.account_email","type":"related_field","label":"Support Contractor Email","column":2,"related":{"table":"accounts","column":"account_email","fk_column":"enrollment_support_contractor_account_id","column_type":"text"}},
      {"name":"enrollment_support_contractor_account_id.account_phone","type":"related_field","label":"Support Contractor Phone Number","column":1,"related":{"table":"accounts","column":"account_phone","fk_column":"enrollment_support_contractor_account_id","column_type":"text"}},
      {"name":"enrollment_support_contractor_account_id.billing_street","type":"related_field","label":"Support Contractor Full Address","column":1,"related":{"table":"accounts","column":"billing_street","fk_column":"enrollment_support_contractor_account_id","column_type":"text"}},
      {"name":"enrollment_support_contractor_account_id.billing_city","type":"related_field","label":"City","column":1,"related":{"table":"accounts","column":"billing_city","fk_column":"enrollment_support_contractor_account_id","column_type":"text"}},
      {"name":"enrollment_support_contractor_account_id.billing_state","type":"related_field","label":"State / Province","column":2,"related":{"table":"accounts","column":"billing_state","fk_column":"enrollment_support_contractor_account_id","column_type":"text"}},
      {"name":"enrollment_support_contractor_account_id.billing_zip","type":"related_field","label":"Postal / Zip Code","column":2,"related":{"table":"accounts","column":"billing_zip","fk_column":"enrollment_support_contractor_account_id","column_type":"text"}}
   ]}'::jsonb),
  ('', v_layout, s_install, 'field_group', 'Installation Building Information', 1, 1,
   '{"fields":[
      ...
      {"name":"building_id.building_stories_of_building","type":"related_field","label":"Total Floors in Building","column":2,"related":{"table":"buildings","column":"building_stories_of_building","fk_column":"building_id","column_type":"number"}},
  ...
  ('', v_layout, s_improve, 'field_group', 'What Work Will Be Completed', 1, 1,
   '{"fields":[
      {"name":"enrollment_work_measures","type":"multiselect","label":"What work will be completed?","help_text":"Improvements must be shown on the model provided to be included.","options":[
  ...
  ('', v_layout, s_improve, 'field_group', 'Improvement Costs', 1, 2,
   '{"fields":[
  ...
  ('', v_layout, s_payment, 'field_group', 'Payment Information', 1, 1,
   '{"fields":[
      ...
      {"name":"enrollment_contractor_account_id.account_fein","type":"related_field","label":"Tax Identification FEIN","column":2,"related":{"table":"accounts","column":"account_fein","fk_column":"enrollment_contractor_account_id","column_type":"text"}},
