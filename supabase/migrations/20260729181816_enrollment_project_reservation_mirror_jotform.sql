-- Enrollment WI-IRA-MF-HOMES-Project-Reservation: mirror the JotForm EXACTLY.
--
-- Refines the layout shipped in 20260729174350 to match the "IRA HOMES
-- Multifamily Project Submittal" JotForm field-for-field, in order, with the
-- form's real dropdown/radio options (captured from the live form). The
-- "Enrollment" section stays as-is for internal use; every section after it is
-- rendered single-column so the vertical order matches the form top-to-bottom.
--
-- Sources: free-form values inherit read-only from the building / property /
-- opportunity / contractor account (related_fields). Fixed-option radio/select
-- questions become real enrollment picklists carrying the form's exact options,
-- because a Chrome extension must map each to a specific form choice.

-- 1) No-parent fields the exact form requires ---------------------------------
ALTER TABLE public.enrollments
  ADD COLUMN IF NOT EXISTS enrollment_application_for uuid,   -- I'm Applying for a(n)
  ADD COLUMN IF NOT EXISTS enrollment_building_type    uuid,  -- Building Type? (Existing / New Construction)
  ADD COLUMN IF NOT EXISTS enrollment_income_level      uuid, -- Which income level (Low-Income / Moderate)
  ADD COLUMN IF NOT EXISTS enrollment_heating_type      uuid, -- How is this building heated?
  ADD COLUMN IF NOT EXISTS enrollment_work_measures      jsonb,-- What work will be completed? (multi-select)
  ADD COLUMN IF NOT EXISTS enrollment_submitted_by       uuid;-- Who is submitting this form?
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='enrollments_submitted_by_fkey') THEN
    ALTER TABLE public.enrollments ADD CONSTRAINT enrollments_submitted_by_fkey
      FOREIGN KEY (enrollment_submitted_by) REFERENCES public.users(id);
  END IF;
END $$;

-- 2) Correct the option sets seeded in 20260729174350 to the form's real ones --
-- building_project_type: Central Multifamily / In-Unit -> the form's four types
UPDATE public.picklist_values SET picklist_is_active=false, picklist_show_in_path=false
 WHERE picklist_object='enrollments' AND picklist_field='building_project_type'
   AND picklist_value IN ('Central Multifamily','In-Unit');
-- payee: all three placeholders were wrong
UPDATE public.picklist_values SET picklist_is_active=false, picklist_show_in_path=false
 WHERE picklist_object='enrollments' AND picklist_field='payee'
   AND picklist_value IN ('Primary Contractor','Building Owner','Property Owner');
-- tax_classification: the renamed/removed values (keep C Corp / S Corp / Partnership)
UPDATE public.picklist_values SET picklist_is_active=false, picklist_show_in_path=false
 WHERE picklist_object='enrollments' AND picklist_field='tax_classification'
   AND picklist_value IN ('Individual / Sole Proprietor','Trust / Estate','Limited Liability Company','Non-Profit / Tax-Exempt');
-- modeling_software: EnergyPlus -> "Energy Plus"
UPDATE public.picklist_values SET picklist_is_active=false, picklist_show_in_path=false
 WHERE picklist_object='enrollments' AND picklist_field='modeling_software' AND picklist_value='EnergyPlus';
-- re-order the tax_classification values we keep to the form's order
UPDATE public.picklist_values SET picklist_sort_order=3 WHERE picklist_object='enrollments' AND picklist_field='tax_classification' AND picklist_value='Partnership';
UPDATE public.picklist_values SET picklist_sort_order=4 WHERE picklist_object='enrollments' AND picklist_field='tax_classification' AND picklist_value='C Corporation';
UPDATE public.picklist_values SET picklist_sort_order=5 WHERE picklist_object='enrollments' AND picklist_field='tax_classification' AND picklist_value='S Corporation';

-- 3) Seed the form's exact option sets ----------------------------------------
INSERT INTO public.picklist_values (picklist_object, picklist_field, picklist_value, picklist_label, picklist_sort_order)
VALUES
  ('enrollments','application_for','Project Reservation','Project Reservation',1),
  ('enrollments','application_for','Final Installation Payment Request','Final Installation Payment Request',2),
  ('enrollments','building_type','Existing','Existing',1),
  ('enrollments','building_type','New Construction','New Construction does not qualify for HOMES rebates',2),
  ('enrollments','building_project_type','Single Family/Townhouse/Rowhouse','Single Family/Townhouse/Rowhouse',1),
  ('enrollments','building_project_type','Multifamily - In Unit','Multifamily - In Unit',2),
  ('enrollments','building_project_type','Multifamily - Central 2-4 Units','Multifamily - Central 2-4 Units',3),
  ('enrollments','building_project_type','Multifamily - Central 5 Units','Multifamily - Central 5 Units',4),
  ('enrollments','income_level','Low-Income','Low-Income',1),
  ('enrollments','income_level','Moderate','Moderate',2),
  ('enrollments','heating_type','Electric','Electric',1),
  ('enrollments','heating_type','Natural Gas','Natural Gas',2),
  ('enrollments','heating_type','Propane','Propane',3),
  ('enrollments','heating_type','Fuel Oil','Fuel Oil',4),
  ('enrollments','heating_type','Wood','Wood',5),
  ('enrollments','payee','Registered Contractor','Registered Contractor',1),
  ('enrollments','payee','Multifamily Building Owner','Multifamily Building Owner',2),
  ('enrollments','payee','In-Unit Owner','In-Unit Owner',3),
  ('enrollments','tax_classification','Individual','Individual',1),
  ('enrollments','tax_classification','Sole Proprietorship','Sole Proprietorship',2),
  ('enrollments','tax_classification','Limited Liability Company (LLC)','Limited Liability Company (LLC)',6),
  ('enrollments','tax_classification','Nonprofit','Nonprofit',7),
  ('enrollments','modeling_software','Energy Plus','Energy Plus',4)
ON CONFLICT DO NOTHING;

-- 4) Rebuild the layout to mirror the JotForm ---------------------------------
DO $$
DECLARE
...
BEGIN
  ...
  UPDATE page_layout_sections SET is_deleted=true,
         deletion_reason='Rebuilt to mirror the IRA HOMES Project Submittal JotForm exactly', updated_at=now()
   WHERE page_layout_id=v_layout AND is_deleted IS NOT TRUE;
  ...
  INSERT INTO page_layout_sections (id, page_layout_id, section_order, section_label, section_columns, section_tab, section_placement, is_deleted) VALUES
    ...
    (s_appl,    v_layout, 2, 'Application',                       1, 'Details', 'main', false),
    (s_contr,   v_layout, 3, 'Primary Contractor Information',    1, 'Details', 'main', false),
    (s_install, v_layout, 4, 'Installation Building Information', 1, 'Details', 'main', false),
    (s_utility, v_layout, 5, 'Utility Information',               1, 'Details', 'main', false),
    (s_improve, v_layout, 6, 'Building Improvements',             1, 'Details', 'main', false),
    (s_payment, v_layout, 7, 'Payment Information',               1, 'Details', 'main', false),
    (s_docs,    v_layout, 8, 'Supporting Documentation',          1, 'Details', 'main', false),
  ...
  ('', v_layout, s_appl, 'field_group', 'Application', 1, 1,
   '{"fields":[
      ...
      {"name":"enrollment_building_type","type":"picklist","display":"radio","label":"Building Type","column":1},
  ...
  ('', v_layout, s_contr, 'field_group', 'Primary Contractor Information', 1, 1,
   '{"fields":[
      ...
      {"name":"enrollment_contractor_contact_id","type":"lookup","label":"Primary Contractor Contact Name","column":1,"lookup_table":"contacts","lookup_field":"contact_name"},
      ...
      {"name":"enrollment_contractor_account_id.account_phone","type":"related_field","label":"Primary Contractor Phone Number","column":1,"related":{"table":"accounts","column":"account_phone","fk_column":"enrollment_contractor_account_id","column_type":"text"}},
      ...
      {"name":"enrollment_contractor_account_id.billing_state","type":"related_field","label":"State","column":1,"related":{"table":"accounts","column":"billing_state","fk_column":"enrollment_contractor_account_id","column_type":"text"}},
      {"name":"enrollment_contractor_account_id.billing_zip","type":"related_field","label":"Postal / Zip Code","column":1,"related":{"table":"accounts","column":"billing_zip","fk_column":"enrollment_contractor_account_id","column_type":"text"}},
      ...
      {"name":"enrollment_support_contractor_account_id","type":"lookup","label":"Support Contractor","column":1,"lookup_table":"accounts","lookup_field":"account_name"}
   ]}'::jsonb),
  ...
  ('', v_layout, s_install, 'field_group', 'Installation Building Information', 1, 1,
   '{"fields":[
      ...
      {"name":"property_id.property_mf_raw_mgmt_contact_full_name","type":"related_field","label":"Contact Name","column":1,"related":{"table":"properties","column":"property_mf_raw_mgmt_contact_full_name","fk_column":"property_id","column_type":"text"}},
      {"name":"property_id.property_hud_management_email","type":"related_field","label":"Email","column":1,"related":{"table":"properties","column":"property_hud_management_email","fk_column":"property_id","column_type":"text"}},
      {"name":"property_id.property_hud_management_phone","type":"related_field","label":"Phone Number","column":1,"related":{"table":"properties","column":"property_hud_management_phone","fk_column":"property_id","column_type":"text"}},
      {"name":"property_id.property_mfown_raw_owner_individual_full_name","type":"related_field","label":"Building Owner Name","column":1,"related":{"table":"properties","column":"property_mfown_raw_owner_individual_full_name","fk_column":"property_id","column_type":"text"}},
  ...
  ('', v_layout, s_utility, 'field_group', 'Utility Information', 1, 1,
   '{"fields":[
      ...
      {"name":"building_id.building_electric_account_number","type":"related_field","label":"Electric Account Number","column":1,"related":{"table":"buildings","column":"building_electric_account_number","fk_column":"building_id","column_type":"text"}},
  ...
  ('', v_layout, s_improve, 'field_group', 'Building Improvements', 1, 1,
   '{"fields":[
      ...
      {"name":"enrollment_total_ira_homes_cost","type":"currency","label":"Total IRA HOMES Cost","column":1},
      {"name":"enrollment_modeled_savings","type":"currency","label":"Modeled Savings Predicted","column":1},
      {"name":"enrollment_estimated_completion_date","type":"date","label":"Estimated Completion Date","column":1}
   ]}'::jsonb),
  ...
  ('', v_layout, s_payment, 'field_group', 'Payment Information', 1, 1,
   '{"fields":[
      ...
      {"name":"enrollment_payment_state","type":"text","label":"State / Province","column":1},
      {"name":"enrollment_payment_zip","type":"text","label":"Postal / Zip Code","column":1}
   ]}'::jsonb),
