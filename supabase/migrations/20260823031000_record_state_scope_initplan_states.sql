-- ============================================================================
-- Geographic (state) record scoping — evaluate the grant list once per query
--
-- A scoped user's queries still timed out, and the cancellation pointed at the
-- culprit: "SQL function app_current_role_name during startup / app_is_admin /
-- app_user_state_scope". A STABLE function called bare in a policy is
-- re-evaluated for every row, so counting 7,536 accounts re-read the user's
-- role and grants 7,536 times — twice over, once per reverse-lookup path.
--
-- Wrapping it in a scalar sub-select makes the planner hoist it into an
-- InitPlan and evaluate it once per query. This is the same shape LEAP's
-- existing policies already use — app_select_accounts reads
-- "( SELECT app_user_can('accounts', 'read') )", not the bare call — so the
-- generated policies now match the platform's established pattern.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    ELSIF r.rsss_resolution_kind = 'polymorphic_lookup' THEN
      -- Inside a resolver the states are already in hand; in a policy the
      -- caller's own scope is read by the two-argument form, which is the only
      -- one `authenticated` may execute.
      IF p_states_expr = 'p_states' THEN
        v_paths := v_paths || format('public.record_in_state_scope(%I.%I, %I.%I, p_states)',
...
BEGIN
  ...
  v_pred := public.build_record_state_scope_predicate(
    p_object, p_object, '(SELECT public.app_user_state_scope())::text[]');
  ...
  EXECUTE format(
    'CREATE POLICY %I ON public.%I AS RESTRICTIVE FOR ALL TO public
       USING ((SELECT public.app_user_state_scope()) IS NULL OR %s)
       WITH CHECK ((SELECT public.app_user_state_scope()) IS NULL OR %s)',
