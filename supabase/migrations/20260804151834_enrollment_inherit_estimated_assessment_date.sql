-- Estimated Assessment Date on the enrollment inherits from the building's
-- assessment record ("Date of IQ Assessment") on the same opportunity. Added to
-- the existing enrollment inheritance trigger (fill-if-null, still gated by the
-- status lock). Only the new assessment-date block differs from the prior
-- version (20260804150000-era).
...
DECLARE
  ...
  v_pt_value text;
BEGIN
  -- LOCK: past the prep/verify stage, the automation is hands-off.
  ...
  -- Estimated Assessment Date inherited from the building's assessment record
  -- (Date of IQ Assessment), preferring the assessment on the same opportunity.
