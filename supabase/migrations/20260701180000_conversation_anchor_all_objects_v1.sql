-- =====================================================================
-- Anchor email/SMS conversations to every business object (v1)
--
-- The conversation resolver previously only set contact/account/project/
-- service_appointment FKs, so an email sent from an Opportunity (or
-- Property, Building, etc.) never pinned to that record and never appeared
-- in its Conversations panel. The panel already supports these FKs
-- (SUPPORTED_FK) -- this closes the write side.
--
-- New optional params are appended after p_subject so existing positional
-- and named callers keep working. The 8-arg version is dropped so named
-- calls resolve unambiguously to this one.
-- =====================================================================

DROP FUNCTION IF EXISTS public.find_or_create_conversation(text, text, text, uuid, uuid, uuid, uuid, text);
