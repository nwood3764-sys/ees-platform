-- =============================================================================
-- Project Portal — projects + work orders under each opportunity
--
-- Extends the building-level tracker with the execution layer:
--   Building → Opportunity → Project[] → Work Order[] (per unit)
-- Each opportunity now carries its projects (projects.opportunity_id), each
-- project carries its work orders (work_orders.project_id), and each work order
-- carries its unit (work_orders.unit_id) + current status. Projects are labeled
-- by project_record_type; work orders by work_order_record_type; statuses come
-- from the project_status / work_order_status picklists (resolved to labels).
-- Opportunity stage bars are unchanged (data-driven per record type).
-- =============================================================================
  ...
  wo_by_project AS (
    SELECT project_id,
      jsonb_agg(jsonb_build_object(
        'id', id, 'name', work_order_name, 'record_type', record_type,
        'status', status_label, 'unit_id', unit_id, 'unit_number', unit_number
      ) ORDER BY unit_number, record_type) AS wos
    FROM scoped_work_orders GROUP BY project_id
  ),
  projects_by_opp AS (
  ...
  opps_by_building AS (
    SELECT so.building_id,
      jsonb_agg(jsonb_build_object(
        ...
        'program', so.program_label, 'stage_label', so.stage_label, 'stage_order', so.stage_order,
