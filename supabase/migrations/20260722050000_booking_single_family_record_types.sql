-- Self-scheduled bookings created the property, building, and project with NO
-- record type. For the NC Energy Savers single-family program these must be
-- single-family. Stamp the single-family record types (resolved by stable
-- value, not hardcoded UUID) on the records the booking RPC creates, and
-- backfill the records already created without one.
--
-- Opportunity record type is intentionally left unset here: the NC single-family
-- HOMES opportunity record types exist but have no stage lifecycle scoped to
-- them yet, so stamping one would leave an empty stage path. That is handled in
-- the HOMES intake workstream once the lifecycle is established.
...
BEGIN
  ...
  -- Single-family record types (resolved by value so no UUID is hardcoded).
  SELECT id INTO v_property_rt FROM picklist_values
   WHERE picklist_object='properties' AND picklist_field='record_type'
     AND picklist_value='single_family' AND picklist_is_active LIMIT 1;
  SELECT id INTO v_building_rt FROM picklist_values
   WHERE picklist_object='buildings' AND picklist_field='record_type'
     AND picklist_value='single_family' AND picklist_is_active LIMIT 1;
  SELECT id INTO v_project_rt FROM picklist_values
   WHERE picklist_object='projects' AND picklist_field='record_type'
     AND picklist_value='single_family_energy_assessment' AND picklist_is_active LIMIT 1;
  ...
  IF v_property_id IS NULL THEN
    ...
    VALUES ('', v_street, v_street, v_city, v_state, v_zip, v_account_id, v_property_rt, v_admin_id,
            now(), v_admin_id, now(), v_admin_id)
  ...
  IF v_building_id IS NULL THEN
    ...
    VALUES ('', v_street, 'Main', v_property_id, v_building_rt, v_admin_id,
            now(), v_admin_id, now(), v_admin_id)
  ...
  VALUES ('', trim(v_first || ' ' || v_last) || ' — assessment',
          v_account_id, v_opportunity_id, v_property_id, v_building_id, v_project_rt, v_admin_id,
          now(), v_admin_id, now(), v_admin_id)
  ...
  INSERT INTO service_appointments (sa_record_number, sa_name, work_type_id, work_order_id, contact_id, sa_owner,
  ...
  VALUES ('', 'SA', v_work_type_id, v_work_order_id, v_contact_id, v_admin_id,
...
-- Backfill single-family record types onto records the booking flow already
-- created without one (limited to booking-origin records: those whose work
-- order carries the publicly-schedulable NC site-visit work type).
WITH booked AS (
  SELECT DISTINCT sa.work_order_id, wo.property_id, wo.project_id, p.building_id
  FROM service_appointments sa
  JOIN work_orders wo ON wo.id = sa.work_order_id
  JOIN work_types wt ON wt.id = wo.work_type_id AND wt.work_type_public_slug = 'nc-energy-savers-site-visit'
  LEFT JOIN projects p ON p.id = wo.project_id
  WHERE coalesce(sa.sa_is_deleted,false) = false
)
UPDATE properties pr
   SET property_record_type = (SELECT id FROM picklist_values WHERE picklist_object='properties' AND picklist_field='record_type' AND picklist_value='single_family' AND picklist_is_active LIMIT 1),
       property_updated_at = now()
  FROM booked b
 WHERE pr.id = b.property_id AND pr.property_record_type IS NULL;

WITH booked AS (
  SELECT DISTINCT bld.id AS building_id
  FROM service_appointments sa
  JOIN work_orders wo ON wo.id = sa.work_order_id
  JOIN work_types wt ON wt.id = wo.work_type_id AND wt.work_type_public_slug = 'nc-energy-savers-site-visit'
  JOIN buildings bld ON bld.property_id = wo.property_id
  WHERE coalesce(sa.sa_is_deleted,false) = false
)
UPDATE buildings b
   SET building_record_type = (SELECT id FROM picklist_values WHERE picklist_object='buildings' AND picklist_field='record_type' AND picklist_value='single_family' AND picklist_is_active LIMIT 1),
       building_updated_at = now()
  FROM booked bk
 WHERE b.id = bk.building_id AND b.building_record_type IS NULL;
...
UPDATE projects pj
   SET project_record_type = (SELECT id FROM picklist_values WHERE picklist_object='projects' AND picklist_field='record_type' AND picklist_value='single_family_energy_assessment' AND picklist_is_active LIMIT 1),
       project_updated_at = now()
  FROM booked b
 WHERE pj.id = b.project_id AND pj.project_record_type IS NULL;
