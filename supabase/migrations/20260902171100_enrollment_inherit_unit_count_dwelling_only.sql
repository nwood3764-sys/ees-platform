-- enrollment_inherit_from_parents filled "units per building" from count(*) of
-- ALL active units, including non-dwelling records (Common Area, Hallway,
-- Mechanical Room, etc.). Count DWELLING-UNIT only, matching
-- recompute_building_rollups and sync_enrollment_unit_counts_from_building.
-- Only the unit-count query changes; the rest is the deployed body verbatim.
-- Already applied to prod via the Supabase MCP; this mirrors it for replays.
...
BEGIN
  ...
  IF NEW.building_id IS NOT NULL THEN
    ...
    -- Dwelling units only -- a "unit in the building" for enrollment purposes is
    -- a dwelling, never a Common Area / Hallway / Mechanical Room / etc.
    SELECT count(*) INTO v_unit_cnt
      FROM public.units u
      JOIN public.picklist_values urt ON urt.id = u.unit_record_type
      WHERE u.building_id = NEW.building_id AND u.unit_is_deleted IS NOT TRUE
        AND urt.picklist_value = 'DWELLING-UNIT';
  END IF;
  ...
  IF NEW.enrollment_estimated_assessment_date IS NULL AND NEW.building_id IS NOT NULL THEN
    SELECT a.assessment_date_of_iq_assessment
      INTO NEW.enrollment_estimated_assessment_date
    FROM public.assessments a
    WHERE a.building_id = NEW.building_id
      AND a.assessment_is_deleted IS NOT TRUE
      AND a.assessment_date_of_iq_assessment IS NOT NULL
    ORDER BY (a.opportunity_id IS NOT DISTINCT FROM NEW.opportunity_id) DESC,
             a.assessment_updated_at DESC NULLS LAST
    LIMIT 1;
  END IF;
