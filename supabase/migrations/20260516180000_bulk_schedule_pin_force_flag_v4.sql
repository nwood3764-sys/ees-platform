-- bulk_schedule_work_orders v4: per-pin force flag.
--
-- A pin with "force": true bypasses the working-window validation
-- (lunch/weekend/daily-bounds) but STILL respects the no-overlap-with-
-- another-pin rule. Use case: dispatcher confirms a late-night or
-- pre-dawn appointment that's outside the standard EES-WI workday
-- but is a real customer commitment. The engine places it where the
-- dispatcher said. Non-pinned WOs still flow only within the regular
-- working window.
--
-- Same signature as v3 (no new params; force is embedded in each
-- pinned_placements entry as an optional jsonb boolean). CREATE OR
...
BEGIN
  ...
  IF jsonb_array_length(p_pinned_placements) > 0 THEN
    ...
    LOOP
      ...
      -- Validate working window unless this pin was forced by the
      -- dispatcher. Forced pins still must not overlap other pins.
...
'Project Scheduler engine v4. Same as v3 but each entry in p_pinned_placements may include an optional "force": true boolean. Forced pins skip the working-window check (lunch/weekend/daily-bounds) and let the dispatcher place a WO at any time of day, while still blocking pin-on-pin overlap.';
