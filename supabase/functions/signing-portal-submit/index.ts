...
//     consent: true,            // ESIGN/UETA consent affirmation
//     tabs: [{ id, value }, ...],   // value = data URL for sig/initial PNG, ISO date string for date, plain text for text
//     decline?: { reason }      // if present, declines the envelope instead of signing
//   }
//
// Outputs (200 JSON):
//   { ok, completed: bool, advanced: bool,
//     next_recipient?: { name, email, signing_url } }
//
// Flow:
//   1. Validate token + envelope state + canSign (it must be this
//      recipient's turn, not already signed/declined, envelope not voided)
//   2. Decline path: mark recipient + envelope Declined, fire events, stop.
//   3. Sign path:
//      a. Update envelope_tabs with values
//      b. Mark recipient signed, capture IP + UA + consent_at
//      c. Download unsigned.pdf, walk ALL filled tabs across ALL
//         recipients, overlay each onto the right page
//      d. Save the resulting PDF to envelopes/{id}/signed.pdf
//      e. Update env_signed_pdf_path so subsequent loads see latest state
//      f. If more recipients waiting → mark next as Sent, return their
//         signing URL. Fire AdvancedToNext.
//      g. If last → mark envelope Completed, generate Certificate of
//         Completion PDF (signatures/envelopes/{id}/certificate.pdf),
//         insert documents row pointing at signed.pdf so the parent
//         record's Documents related-list picks it up. Fire Completed.
// =============================================================================
...
Deno.serve(async (req) => {
  ...
  // ── Validate the token + envelope state ─────────────────────────────
  const { data: recipient } = await supabase
    ...
    .select(`
      ...
      envelope:envelope_id (
        ...
        document_template_id, document_template_snapshot_id, env_name,
  ...
  // ── SIGN path ───────────────────────────────────────────────────────
  if (!body.consent)
    return json({ error: "ESIGN consent required to sign" }, 400)
  if (!Array.isArray(body.tabs))
    return json({ error: "tabs[] required" }, 400)

  // Update tab values for this recipient
  ...
  // ── Build the rolling overlay PDF from unsigned + ALL filled tabs ───
  ...
  // ── Determine if more recipients waiting ────────────────────────────
  ...
  // Insert a documents row pointing at the signed PDF so the parent
  // record's Documents related-list picks it up. The signed PDF and
  // certificate live side-by-side in the signatures bucket.
...
// ─── PDF overlay builder ────────────────────────────────────────────────
// Loads unsigned.pdf fresh, then walks ALL envelope_tabs with non-null
// tab_filled_value across all recipients and draws each onto the right
// page. Idempotent — running twice yields the same result. Stored at
// envelopes/{id}/signed.pdf and used as the "current state" PDF
// throughout the signing flow (recipient N sees recipient 1..N-1's
// overlays). On envelope completion this same file is the final
// signed PDF.
  ...
  for (const t of (tabs || [])) {
    ...
    if (tabType === "signature" || tabType === "initial") {
      ...
      try {
        ...
        // Fit-into-box preserving aspect ratio, centered horizontally,
      ...
      // text and any unknown type render as plain text
...
// Single-page PDF (or multi-page if many events) summarizing the
// envelope's chain-of-custody. Drawn in pdf-lib using StandardFonts so
// no extra font assets are needed. Layout: header, recipient block per
// signer with name/email/order/IP/UA/timestamps, then a chronological
// audit-trail table of every envelope_events row.
...
function formatDate(v: string): string {
  // Prefer ISO → en-US short. Falls back to raw.
  try {
    const d = new Date(v)
