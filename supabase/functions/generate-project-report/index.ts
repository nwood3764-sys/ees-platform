...
//
// Authors of custom_text sections write a body template that may include
// double-curly placeholders. At render time we walk the project graph and
// substitute. The placeholder language is intentionally minimal — paths
// only, no conditionals, no loops. Custom_text is meant for cover letters,
// intro paragraphs, sign-offs — not full templating.
//
// Resolution order (longest-prefix first; first non-null wins):
//
//   project.*          fields off the Project row. Picklist UUID fields
//                      (project_status, project_record_type) auto-resolve
//                      to their picklist_label rather than printing UUIDs.
//                      Special: project.full_record (combined "PROJ-… —
//                      Name").
//   property.*         fields off the Property row. Special:
//                      property.full_address (multi-line composed address).
//   account.*          fields off the Account row.
//   report.*           computed report-level metadata: generated_at,
//                      generated_at_long, generated_by, template_name,
//                      template_record_number, template_version,
//                      work_order_count, work_step_count, photo_count.
//   user.*             the caller (Generated by). first_name, last_name,
//                      full_name, email.
//   today / today.long Convenience date stamps.
//
// Unknown placeholders (typos, paths off non-existent objects) render as
// "[unknown: {{path}}]" in muted color — this is a deliberate choice so
// typos don't silently disappear. Empty / null field values render as a
// single dash to match the rest of the report.
// ───────────────────────────────────────────────────────────────────────────
...
function resolveMergeField(ctx: RenderCtx, path: string): { value: string, found: boolean } {
  ...
  if (root === "report") {
    ...
    for (const arr of ctx.photosByWO.values())   totalPhotos += arr.length
    switch (rest) {
      case "generated_at":          return { value: ctx.generatedAt.toLocaleString(), found: true }
      case "generated_at_long":     return { value: ctx.generatedAt.toLocaleString("en-US", { dateStyle: "long", timeStyle: "short" }), found: true }
      case "generated_at_date":     return { value: ctx.generatedAt.toLocaleDateString(), found: true }
      case "generated_by":          return { value: ctx.generatedByName, found: true }
      case "template_name":         return { value: ctx.prt.prt_name || "—", found: true }
      case "template_record_number":return { value: ctx.prt.prt_record_number || "—", found: true }
      case "template_version":      return { value: String(ctx.prt.prt_version), found: true }
      case "work_order_count":      return { value: String(ctx.workOrders.length), found: true }
      case "work_step_count":       return { value: String(totalSteps), found: true }
      case "photo_count":           return { value: String(totalPhotos), found: true }
      case "watermark_choice":      return { value: ctx.watermarkChoice === "watermarked" ? "Watermarked" : "Original", found: true }
      default:                      return { value: `[unknown: {{${path}}}]`, found: false }
    }
  }
  if (root === "user") {
    // Composite from generatedByName + email; no separate first/last lookup
    // (we don't keep that on RenderCtx). full_name and email come straight
    // from the values already resolved at request entry.
    switch (rest) {
      case "full_name":  return { value: ctx.generatedByName, found: true }
      case "email":      return { value: ctx.generatedByEmail || ctx.generatedByName, found: true }
      default:           return { value: `[unknown: {{${path}}}]`, found: false }
    }
  }
  if (path === "today")        return { value: ctx.generatedAt.toLocaleDateString(), found: true }
  if (path === "today.long")   return { value: ctx.generatedAt.toLocaleDateString("en-US", { dateStyle: "long" }), found: true }
  if (path === "today.short")  return { value: ctx.generatedAt.toLocaleDateString(), found: true }
  if (path === "today.iso")    return { value: ctx.generatedAt.toISOString().slice(0, 10), found: true }

  return { value: `[unknown: {{${path}}}]`, found: false }
}

// Substitute every {{path}} occurrence in `body`. Returns the substituted
// string plus a flag indicating whether any unknown placeholders were
// encountered (so callers could optionally surface a warning).
function substituteMergeFields(ctx: RenderCtx, body: string): { text: string, hadUnknown: boolean } {
  let hadUnknown = false
  // Match {{ path }} with optional surrounding whitespace. Path is
  // alphanum + dots + underscores. Anything else (newlines, JSON syntax)
  // is left alone.
  const text = body.replace(/\{\{\s*([A-Za-z_][A-Za-z0-9_.]*)\s*\}\}/g, (_match, path: string) => {
    const r = resolveMergeField(ctx, path)
    if (!r.found) hadUnknown = true
    return r.value
  })
  return { text, hadUnknown }
}
...
async function renderSection(ctx: RenderCtx, section: PRTS) {
  switch (section.section_type_value) {
    ...
    case "custom_text": {
      ...
      if (section.prts_section_title && section.prts_section_title.trim()) {
        drawText(ctx.cur, section.prts_section_title, { size: 14, bold: true })
        drawDivider(ctx.cur)
      }
      const rawBody = section.prts_body_template || ""
      const { text } = substituteMergeFields(ctx, rawBody)
      // Render with normal body sizing. Unknown placeholders end up inline
      // as "[unknown: {{path}}]" — visible to the author but not styled
      // separately, which keeps the substitution logic regex-only and
      // avoids per-segment color switching mid-paragraph.
      drawText(ctx.cur, text, { size: 11 })
      return
    }
    default: {
