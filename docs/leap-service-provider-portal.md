...
- **2026-07-29 — Onboarding pipeline reframe SHIPPED (PRs #345–#350, all merged to master/prod same day).** Nicholas: the apply→auto-accept flow was "not acceptable" — the SP tab must be a dashboard, review must be a real multi-step onboarding process with communication tracking, staff must initiate the invite, and the intake must be a designed funnel (multi-trade, upload a COI/W-9, draw a service-area map — not a wall of fields). Delivered in five additive increments:
  - **Staff-initiated invite + New Application + stage dashboard (PR #345):** pipeline stages widened to Submitted → Under Review → Info Requested → Interview → Onboarding → Training → Approved/Declined (migration `20260728180000`, `advance_service_provider_application`); `approve` split from `send invite` (`approveWithoutInvite` + `sendPortalInvite`); staff can start an application manually (`create_service_provider_application` granted to authenticated, `20260728190000`). `ServiceProviderModule` rebuilt as a stage-tile dashboard (click-to-filter) over review cards.
  - **Communication trail (PR #346):** every application carries a full activity log via the shared `log_activity`/`list_activities_for_record` layer (anchored on `service_provider_applications`). Staff log Call/Email/Meeting/Site Visit/Text/Note; every pipeline action auto-records a trail entry.
  - **Multi-trade + COI (PR #348, migration `20260729221717`):** `service_provider_trades` (SPT-) junction (one row per trade, one primary; account/application keep the single primary trade for existing pickers); `account_coi_document_id`/`spa_coi_document_id`. `create_service_provider_application` rewritten additively for `service_provider_types[]` + a `coi` file; intake funnel reordered (company → contact → **trades multi-select** → license & **COI upload** → coverage → review); edge fn `service-provider-intake` v2 (generic `uploadDoc` for W-9 **and** COI). Review card shows all trades + View COI.
  - **Draw-a-boundary service-area map (PR #349):** `src/components/ServiceAreaMap.jsx` — Leaflet + OSM tiles, click-to-draw polygons, turf point-in-polygon against a bundled ZIP-centroid asset (`public/geo/us-zip-centroids.json`, 33,791 ZIPs, generated by `scripts/gen-zip-centroids.mjs` from `us-zips`). Fully client-side (no prod geodata), lazy chunk (~47 KB gzip). Intake unions drawn + manually typed ZIPs.
  - **Onboarding checklist gating the invite (PR #350, migration `20260729223734`):** `service_provider_onboarding_step_templates` (SPOT-, configurable defaults: documents reviewed → initial interview → onboarding session → training completed) + `service_provider_onboarding_steps` (SPOS-, per-application, who/when/notes). RPCs `ensure_service_provider_onboarding_steps` (idempotent) / `set_service_provider_onboarding_step`. Card shows the checklist with progress; **Send portal invite** soft-gates on required steps (confirm to override). Completing a step logs to the comms trail.
  - Help articles **HA-00133** (reviewing/onboarding) and **HA-00137** (public intake) rewritten. Advisor categories unchanged (new SECURITY DEFINER RPCs carry only the known executable lint; all new tables RLS'd with 4 policies).
  - **Still open (needs Nicholas / content):** the *applicant-facing* onboarding + training portal (interview scheduling UI, training modules) — the staff-side pipeline, checklist, and comms are built, but the customer-facing training experience needs content/product decisions before it's built. WI intake rollout is config once NC is validated.
...
- **2026-07-22 — "Issue to Provider" action SHIPPED to branch → the loop is now fully staff-driven.** Nicholas: button on the **work order**. Added via the per-object action registry (`src/data/recordActions.js` `ISSUE_TO_PROVIDER`, scoped to `work_orders`) + `RecordDetail` handler/modal wiring + new `src/components/IssueToProviderModal.jsx` (picks an active service provider, calls `generate_service_provider_proposal` with `p_state: null` so the RPC defaults state from the property) + `fetchActiveServiceProviders`/`issueWorkOrderToProvider` in `serviceProviderService.js`. Pure frontend (the RPC was already live). `build:safe` green.

**The full loop is now operational end to end:** public signup → review queue (Approve & invite / Decline) → account active + auto-emailed portal invite → provider accepts a work order's priced proposal → agreed payout locks → invoices/payments. The only future enhancements left are optional (project-level bundled proposals, provider multi-trade, tier-gating hardening, WI intake rollout — none blocking).
