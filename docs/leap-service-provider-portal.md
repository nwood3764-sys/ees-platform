...
- **2026-07-22 — Phase 4a (public intake) SHIPPED to branch.** Migration `20260722120000_service_provider_intake.sql` (applied + verified on prod) + the anon edge function + the public form.
  ...
  - **Remaining (Phase 4b, connective glue):** internal review/approval of an application → activate the account (`account_service_provider_is_active=true`, status 'Service Provider Active') + provision the provider portal login (create `portal_users` record_type 'Provider User' + `portal_user_account_id` + send the auth invite, cloning `invite-portal-user` with redirect `/provider-portal`); and a staff **"Issue to Provider"** action calling `generate_service_provider_proposal`. These close the loop between intake → portal.
