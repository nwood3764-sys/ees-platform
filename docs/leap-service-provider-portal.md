...
| D3 | Payment model | **Dedicated payout price book**, mirroring the sell-side `price_books`/`price_book_entries` pattern — **regional (state)** with an optional **per-provider override**. Payout = installed quantity × resolved payout rate. **DECIDED** |
