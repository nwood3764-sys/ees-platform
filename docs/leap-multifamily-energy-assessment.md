...
Its sections are laid out to **mirror the DOE Audit Template report** (PNNL, ANSI/ASHRAE/ACCA Standard 211-2018), taken from the real report for 101-111 Queens Court, Rocky Mount: the same six parts in the same order (Overview · Contact Information and Audit Details · Facility Description · Utility Data and Benchmarking · Energy Savings Opportunities · Attachments) with the same sub-section names, so the two documents can be read side by side and the systems photos sit against the same systems in both. Note that Audit Template orders **Windows before Foundation Types** and **Lighting before HVAC**, which is why the report is not in the work plan's own walk order.
