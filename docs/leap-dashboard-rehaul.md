...
*Status 2026-07-26 — SHIPPED at 25 components:* added KPI Card, Stat + Sparkline, Stacked/Clustered/100% Stacked Bar, Area (time), Pyramid, Treemap, Waterfall, Heatmap, Scatter, Histogram, Image; content widgets now render in the runner. Palette search box removed per Nicholas (browsable category list). Phase A remainder also shipped same day: table column picker (`multi-field` inspector control)/sort/totals, metric-KPI-stat conditional color breakpoints (emerald/amber/sky), reference lines + axis titles. Still open from the original P1/P2 list: matrix/pivot table (TanStack), on-canvas filter widgets, combo chart, ribbon, sankey, calendar heatmap, state timeline, box plot, geo map, multi-row card, rating, link widget.
