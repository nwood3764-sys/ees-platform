...
*Status 2026-07-26 — SHIPPED (migration `20260726210000`, applied to prod, verified live, advisors unchanged at 205):* `report_aggregate_2d` / `report_aggregate_time` / `report_aggregate_single`, sharing `report_aggregate`'s semantics via `_report_agg_where`/`_report_agg_label`/`_report_agg_measure` helpers. Client: `runWidgetData` in `reportsService` routes every widget type to its shape (see `WIDGET_QUERY_SHAPES`) with row-fetch fallback; DashboardRunner + LiveWidgetPreview both use it. Scatter/histogram deliberately compute client-side from report rows (no RPC needed at current volumes).

**Phase C — Component expansion to 25+** (registry entries riding A+B), in priority order from the vendor research:
*Status 2026-07-26 — SHIPPED at 25 components:* added KPI Card, Stat + Sparkline, Stacked/Clustered/100% Stacked Bar, Area (time), Pyramid, Treemap, Waterfall, Heatmap, Scatter, Histogram, Image; content widgets now render in the runner. Palette search box removed per Nicholas (browsable category list). Phase A remainder also shipped same day: table column picker (`multi-field` inspector control)/sort/totals, metric-KPI-stat conditional color breakpoints (emerald/amber/sky), reference lines + axis titles.
*Status 2026-07-26 round 2 — SHIPPED at **43 components** (Nicholas: "include absolutely everyone from Salesforce, Tableau, BI… stop cutting this short"):* added **Speedometer Gauge** (the "gas gauge": needle + sky/amber/emerald segment bands from cond_low/cond_high, defaults to thirds of scale — no red, per design system), Bullet/Linear Gauge (DOM-rendered bands + measure bar + target tick), Progress Ring, Rating, Multi-Row Card, Combo (bar+line, opt-in dual axis), Pareto, Rose, Radar (2D pivot → indicator polygons), Sunburst (group→series two-ring), Sankey, Calendar Heatmap (day buckets on a real calendar), Box Plot (client-side quartiles), Matrix/Pivot (row×column totals + value-shaded cells — the highlight table), **on-canvas filter widgets** (Picklist/Toggle/Date Range — values via `dashboard_filter_distinct_values`, state in DashboardRunner, applied like header filters incl. override semantics, Power BI slicer behavior: apply-on-change), and Link. New query routing: `combo` (two aggregates merged by label) and `filter` shapes in `runWidgetData`. **Deliberately NOT built, with reasons:** geo map (needs a bundled US-states GeoJSON asset + `registerMap` — next session), state timeline (LEAP has no per-record status-interval history table), packed bubbles (no native ECharts type), decomposition tree / smart narrative / Q&A (AI-class features → LEAP AI assistant workstream), ribbon (niche; revisit on demand).
